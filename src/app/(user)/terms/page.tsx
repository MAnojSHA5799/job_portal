          ...
          <div className="flex flex-col lg:flex-row gap-16">
            ...
            <div className="lg:w-3/4 space-y-24">
              ...
              <motion.section id="acceptance" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <p className="text-slate-600 leading-relaxed text-lg">{content.intro}</p>
              </motion.section>
              ...
              <motion.section id="license" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <p className="text-slate-600 leading-relaxed text-lg mb-8">{content.license.intro}</p>
                <div className="space-y-4">
                  {content.license.points.map((point, i) => (
                    <div key={i} className="flex gap-6 p-6 rounded-[24px] border border-slate-100 hover:bg-slate-50 transition-colors">
                      <div className="w-2 h-2 rounded-full bg-amber-500 mt-2.5 flex-shrink-0" />
                      <p className="text-slate-600 leading-relaxed text-sm font-medium">{point}</p>
                    </div>
                  ))}
                </div>
              </motion.section>
              ...
              <motion.section id="disclaimer" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <p className="text-slate-600 leading-relaxed text-lg mb-8">{content.disclaimer.intro}</p>
                <div className="p-10 bg-amber-50 rounded-[40px] text-amber-900 relative overflow-hidden shadow-sm">
                  <div className="relative z-10 space-y-6">
                    ...
                    <p className="text-amber-800/80 leading-relaxed text-lg">{content.disclaimer.accuracy}</p>
                  </div>
                </div>
              </motion.section>
              ...
              <motion.section id="limitations" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <p className="text-slate-600 leading-relaxed text-lg mb-12">{content.limitations.text}</p>
                <div className="p-12 rounded-[40px] bg-slate-950 text-center space-y-6 relative overflow-hidden shadow-2xl shadow-slate-900/20">
                  ...
                  <div className="space-y-3 relative z-10">
                    ...
                    <p className="text-slate-400 text-lg leading-relaxed max-w-2xl mx-auto">{content.limitations.governing_law}</p>
                  </div>
                </div>
              </motion.section>
