          ...
          {showFilters && (
            <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-xl shadow-xl shadow-gray-200/50 border border-gray-100 p-5 z-50">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5 block">Job Type to Scrape</label>
                  <select 
                    value={scrapeFilters.jobType}
                    onChange={(e) => setScrapeFilters({...scrapeFilters, jobType: e.target.value})}
                    className="w-full h-10 px-3 rounded-lg border-gray-200 text-sm font-medium focus:ring-indigo-100"
                  >
                ...
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5 block">Data Horizon</label>
                  <select 
                    value={scrapeFilters.jobAge}
                    onChange={(e) => setScrapeFilters({...scrapeFilters, jobAge: e.target.value})}
                    className="w-full h-10 px-3 rounded-lg border-gray-200 text-sm font-medium focus:ring-indigo-100"
                  >
                    <option>Scrape All Jobs</option>
                    <option>New Jobs (Last 24h)</option>
                    <option>Recent Jobs (Last 7 Days)</option>
                  </select>
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5 block">Target Selection</label>
                  <select 
                    value={scrapeFilters.target}
                    onChange={(e) => setScrapeFilters({...scrapeFilters, target: e.target.value})}
                    className="w-full h-10 px-3 rounded-lg border-gray-200 text-sm font-medium focus:ring-indigo-100"
                  >
                    <option>All Active Companies</option>
                    <option>Pending Companies</option>
                  </select>
                </div>

                <div className="pt-2">
                  <Button 
                    className="w-full bg-emerald-500 hover:bg-emerald-600 font-bold shadow-lg shadow-emerald-100 h-10"
