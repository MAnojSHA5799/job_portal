...
function saveJobsNow(results) {
  const indianCities = [
    ...
    'khopoli', 'tarapur', 'jharkhand', 'odisha', 'orissa', 'multiple locations'
  ];
  const uaeCities = [
...
(async () => {
  ...
  for (const listingUrl of jobUrls) {
    ...
    try {
      ...
      else if (type === 'atherenergy') { await scrapeAtherEnergy(page, context, listingUrl, jobsBatch); }
      else if (type === 'spglobal') { await scrapeSpGlobal(page, context, listingUrl, jobsBatch); }
      else if (type === 'tataprojects') { await scrapeTataProjects(page, context, listingUrl, jobsBatch); }
      ...
      else if (type === 'ge_vernova') { await scrapeGeVernova(page, context, listingUrl, jobsBatch); }
      else if (type === 'abb') { await scrapeAbb(page, context, listingUrl, jobsBatch); }
      else if (type === 'teva') { await scrapeTeva(page, context, listingUrl, jobsBatch); }
      else if (type === 'eightfold') { await scrapeEightfold(page, context, listingUrl, jobsBatch); }
      ...
      else if (type === 'bradken') { await scrapeBradken(page, context, listingUrl, jobsBatch); }
      else if (type === 'systra') { await scrapeSystra(page, context, listingUrl, jobsBatch); }
      else if (type === 'dzconnex') { await scrapeDZConnex(page, context, listingUrl, jobsBatch); }
      else if (type === 'skf') { await scrapeSkf(page, context, listingUrl, jobsBatch); }
      else if (type === 'apotex') { await scrapeApotex(page, context, listingUrl, jobsBatch); }
      else if (type === 'motherson') { await scrapeMotherson(page, context, listingUrl, jobsBatch); }
      else if (type === 'kiaindia') { await scrapeKiaIndia(page, context, listingUrl, jobsBatch); }
      else if (type === 'macegroup') { await scrapeMacegroup(page, context, listingUrl, jobsBatch); }
      ...
      else if (type === 'carrier') { await scrapeCarrier(page, context, listingUrl, jobsBatch); }
      else if (type === 'whirlpool') { await scrapeWhirlpool(page, context, listingUrl, jobsBatch); }
      else if (type === 'ericsson') { await scrapeEricsson(page, context, listingUrl, jobsBatch); }
      else if (type === 'ttcportals') { await scrapeTtcPortals(page, context, listingUrl, jobsBatch); }
      else if (type === 'workable_jobs') { await scrapeWorkableJobs(page, context, listingUrl, jobsBatch); }
      ...
      else if (type === 'ripplehire') { await scrapeRipplehire(page, context, listingUrl, jobsBatch); }
      else if (type === 'nestle') { await scrapeNestle(page, context, listingUrl, jobsBatch); }
      else if (type === 'bharatwireropes') { await scrapeBharatWireRopes(page, context, listingUrl, jobsBatch); }
      else if (type === 'gm') { await scrapeGm(page, context, listingUrl, jobsBatch); }
      else if (type === 'airindia') { await scrapeAirIndia(page, context, listingUrl, jobsBatch); }
      else if (type === 'deere') { await scrapeDeere(page, context, listingUrl, jobsBatch); }
      else if (type === 'qualcomm') { await scrapeQualcomm(page, context, listingUrl, jobsBatch); }
      else if (type === 'slb') { await scrapeSlb(page, context, listingUrl, jobsBatch); }
      else if (type === 'godrej') { await scrapeGodrej(page, context, listingUrl, jobsBatch); }
      else if (type === 'bosch') { await scrapeBosch(page, context, listingUrl, jobsBatch); }
      else if (type === 'continental') { await scrapeContinental(page, context, listingUrl, jobsBatch); }
      else if (type === 'dana') { await scrapeDana(page, context, listingUrl, jobsBatch); }
      else if (type === 'dayforce') { await scrapeDayforce(page, context, listingUrl, jobsBatch); }
...
async function detectTypeFromDom(page) {
  return await page.evaluate(() => {
    ...
    if (url.includes('careers.godrejindustries.com')) return 'godrej';
...
async function scrapeSpGlobal(page, context, listingUrl, results) {
  ...
  while (true) {
    ...
    // Pagination — click "Next Page" button if not disabled
    const hasNext = await page.evaluate(() => {
...
async function scrapeTataProjects(page, context, listingUrl, results) {
  ...
  while (true) {
    ...
    const hasNext = await page.evaluate(() => {
      ...
      if (nextLink && nextLink.href) { nextLink.click(); return true; }
      return false;
    });

    if (!hasNext) { console.log(`  ✅ Tata Projects done — ${pageNum} pages`); break; }
    await page.waitForTimeout(3000);
    pageNum++;
  }
}
...
async function scrapeCaterpillarAllPages(firstPage, context, results) {
  ...
  while (true) {
    ...
    const nextUrl = await currentPage.evaluate(() => {
      ...
      // Fallback: bump ?page= in the current URL if more results remain.
...
async function scrapeSlb(page, context, listingUrl, results) {
  ...
  try { await page.close(); } catch (e) { } // Clean up the listing page
...
async function scrapeGodrej(page, context, listingUrl, results) {
  ...
  while (true) {
    ...
    const jobs = await page.evaluate(() => {
      ...
      return links.map(a => {
        ...
        const jobId = a.getAttribute('data-ph-at-job-id-text')
          ...
          || '';
        const href = a.href || '';
        return { title, jobId, detailUrl: href, company: 'Godrej', location: '', date: '' };
      }).filter(j => j.detailUrl && j.title !== 'Not Found');
    });
    ...
    // Wait for URL to update (Phenom uses ?from=N&s=1)
    try {
...
function genericJobEvaluator() {
  ...
  try {
    if (window.location.href.toLowerCase().includes('jobs.continental.com')) {
      ...
      const applyBtn = document.querySelector('a.c-jobdetails__apply, a[href*="smartrecruiters.com"]');
      const applyUrl = applyBtn ? applyBtn.href : window.location.href;

      const descEl = document.querySelector('.c-jobdetails__section-wrapper') || document.querySelector('main');
