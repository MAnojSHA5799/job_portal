...
function saveJobsNow(results) {
    ...
    const filtered = results.filter(job => {
        ...
        for (const fi of filterArray) {
            ...
            if (nf === 'india') {
                if (indianCities.some(c => loc.includes(c)) || inUrl || isPorscheIndiaUrl) { locMatch = true; break; }
...
async function visitDetailPage(context, job, source, results, extra = {}) {
    ...
    try {
        ...
        if (!finalSalary || finalSalary === 'Not Available' || finalSalary === 'Not Found') {
            ...
            // Match patterns: $73,800 - $132,800 | ₹12,00,000 | AED 15,000 | £50,000 | €60,000
            // IMPORTANT: \d[\d,]* means first char after currency MUST be a digit (prevents 'rs,' '€,' false matches)
            const salaryPatterns = [
                /\bAED\s*\d[\d,]*(?:\s*[-\u2013]\s*(?:AED)?\s*\d[\d,]*)?(?:\s*(?:per\s+(?:month|annum|year|yr)|\/(?:month|yr|year|annum)))?/i,
                /(?:\u20b9|INR)\s*\d[\d,]{3,}(?:\s*[-\u2013]\s*(?:\u20b9|INR)?\s*\d[\d,]+)?(?:\s*(?:per\s+(?:month|annum|year|yr)|\/(?:month|yr|year|annum|pa)))?(?:\s*(?:lakh|lakhs|LPA))?/i,
                /\bRs\.?\s*\d[\d,]{3,}(?:\s*[-\u2013]\s*Rs\.?\s*\d[\d,]+)?(?:\s*(?:per\s+(?:month|annum|year|yr)|\/(?:month|yr|year|annum|pa)))?(?:\s*(?:lakh|lakhs|LPA))?/i,
                /\$\d[\d,]*(?:\.\d+)?(?:\s*[-\u2013]\s*\$\d[\d,]*(?:\.\d+)?)?(?:\s*(?:per\s+(?:hour|hr|month|annum|year)|(?:hour|hr|yr|year|annum|annually)))?/i,
                /[\u00a3\u20ac]\s*\d[\d,]*(?:\.\d+)?(?:\s*[-\u2013]\s*[\u00a3\u20ac]?\s*\d[\d,]*(?:\.\d+)?)?(?:\s*(?:per\s+(?:annum|year|month)|\/(?:yr|year|annum|pa)))?/i,
