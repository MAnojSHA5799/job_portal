  ...
  const jobs = await page.evaluate(() => {
      ...
      function getFieldText(el) {
        ...
        if (sh) {
          ...
          if (parts.length) return [...parts].map(p => p.textContent?.trim()).filter(Boolean).join(', ');
      ...
      for (const result of atomicResults) {
        ...
        if (atomicLink) {
          const linkShadow = atomicLink.shadowRoot;
          const anchor = linkShadow?.querySelector('a');
          href = anchor?.href || '';
          title = anchor?.textContent?.trim() || '';
        }
        if (!href) {
          const anyAnchor = shadow.querySelector('a[href*="/job/"]') || shadow.querySelector('a[href*="careers.slb.com"]');
          if (anyAnchor) { href = anyAnchor.href; title = title || anyAnchor.textContent?.trim(); }
        }
      ...
      return {
          atomicResultListExists: !!document.querySelector('atomic-result-list'),
          atomicResultListShadowExists: !!document.querySelector('atomic-result-list')?.shadowRoot,
          numAtomicResults: atomicResults.length,
          jobs: pageResults
      };
  });
