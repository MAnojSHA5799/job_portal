...
(async () => {
  ...
  const jobs = await page.evaluate(() => {
      ...
      for (const result of atomicResults) {
        ...
        let linkInnerHtml = '';
        let anchorHTML = '';
        let foundAtomicText = false;
        
        if (atomicLink) {
          const linkShadow = atomicLink.shadowRoot;
          if (linkShadow) {
              linkInnerHtml = linkShadow.innerHTML;
              const anchor = linkShadow.querySelector('a');
              href = anchor?.href || '';
              if (anchor) {
                  anchorHTML = anchor.innerHTML;
              }
              const atomicText = linkShadow.querySelector('atomic-text') || linkShadow.querySelector('atomic-result-text atomic-text');
              if (atomicText) {
                  foundAtomicText = true;
        ...
        pageResults.push({ title, href, city, country, category, linkInnerHtml, anchorHTML, foundAtomicText });
      }
      return pageResults;
  });
